...
/** Layout tab controls for card widgets. */
    ...
    <div className="space-y-3">
      <Group label="Text align">
        {(["left", "center", "right"] as const).map((a) => (
          <Chip key={a} active={(s.align ?? "left") === a} onClick={() => set({ align: a })}>
            {a}
          </Chip>
        ))}
      </Group>
      <Group label="Padding">
        {(["compact", "normal", "roomy"] as const).map((p) => (
          <Chip key={p} active={(s.padding ?? "normal") === p} onClick={() => set({ padding: p })}>
            {p}
          </Chip>
        ))}
      </Group>
      <Group label="Width">
        {(["full", "inset"] as const).map((w) => (
          <Chip key={w} active={(s.width ?? "full") === w} onClick={() => set({ width: w })}>
            {w}
          </Chip>
        ))}
      </Group>
      <p className="text-[10px] leading-snug text-brand-muted">
        Layout only affects this section on the public stack. Reorder blocks with the drag handle on the
        canvas.
      </p>
    </div>
  );
