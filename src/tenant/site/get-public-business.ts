...
async function toBusiness(
  ...
  return {
    ...
    customerCancelOrders: row.customerCancelOrders,
    customerModifyOrders: row.customerModifyOrders,
    catalogMode: (row.catalogMode as Business["catalogMode"]) ?? "both",
    deliveryOps: (row.deliveryOps as Business["deliveryOps"]) ?? "manual",
